...
class MySuperAnimCompiler {
  ...
  Map<Vector2, List<dVector2>> internalObjs = {};
  Map<Vector2, List<dVector2>> internalObjsLoop = {};
      ...
      if (grp.objects.isNotEmpty) {
        ...
        if (obj.isPolygon) {
          ...
          for (final point in obj.polygon) {
            internalObjsLoop[position]?.add(dVector2(point.x + obj.x + position.x, point.y + obj.y + position.y));
          }
        }
        if (obj.isPolyline) {
          internalObjs.putIfAbsent(position, () => []);
          for (final point in obj.polyline) {
            internalObjs[position]?.add(dVector2(point.x + obj.x + position.x, point.y + obj.y + position.y));
          }
        }
        if (obj.isRectangle) {
          internalObjsLoop.putIfAbsent(position, () => []);
          internalObjsLoop[position]?.add(dVector2(obj.x + position.x, obj.y + position.y));
          internalObjsLoop[position]?.add(dVector2(obj.x + position.x, obj.y + obj.height + position.y));
          internalObjsLoop[position]?.add(dVector2(obj.x + obj.width + position.x, obj.y + obj.height + position.y));
          internalObjsLoop[position]?.add(dVector2(obj.x + obj.width + position.x, obj.y + position.y));
        }
      }
    }
  }

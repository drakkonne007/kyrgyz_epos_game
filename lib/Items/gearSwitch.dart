...
class GearSwitch extends SpriteAnimationComponent with HasGameRef<KyrgyzGame>
{
  ...
  void changeState() async{
    if (isClosed) {
      animation = toOpen;
      await gameRef.dbHandler.changeItemState(id: _targetId, opened: true,worldName: gameRef.gameMap.currentGameWorldData!.nameForGame);
    } else {
      animation = toOpen.reversed();
      await gameRef.dbHandler.changeItemState(id: _targetId, opened: false,worldName: gameRef.gameMap.currentGameWorldData!.nameForGame);
    }
    isClosed = !isClosed;
  }
}